...
class NuscData(torch.utils.data.Dataset):
    def __init__(self, nusc, is_train, data_aug_conf, centroid=None, bounds=None, res_3d=None, nsweeps=1, seqlen=1,
                 refcam_id=1, get_tids=False, temporal_aug=False, use_radar_filters=False, do_shuffle_cams=True):
        self.nusc = nusc
    ...
    def get_center_and_offset_bev(self, lrtlist_cam, seg_bev):
        ...
        radius = 3
        center, offset = self.vox_util.xyz2circles_bev(clist_cam, radius, self.Z, self.Y, self.X, already_mem=False,
                                                       also_offset=True)
        ...
        return center.squeeze(0), offset.squeeze(0), size_bev.squeeze(0), ry_bev.squeeze(0), ycoord_bev.squeeze(
            0)  # 1,Z,X; 2,Z,X; 3,Z,X; 1,Z,X
...
class VizData(NuscData):
    ...
    def get_single_item(self, index, cams, refcam_id=None):
        ...
        if lidar_data.shape[0] > V:
            ...
            lidar0_extra = np.pad(lidar0_extra, [(0, V // self.nsweeps - lidar0_extra.shape[0]), (0, 0)],
                                  mode='constant')
            lidar_data = np.pad(lidar_data, [(0, V - lidar_data.shape[0]), (0, 0)], mode='constant')
            lidar_extra = np.pad(lidar_extra, [(0, V - lidar_extra.shape[0]), (0, 0)], mode='constant',
                                 constant_values=-1)
        lidar0_data = np.transpose(lidar0_data)
        lidar0_extra = np.transpose(lidar0_extra)
        lidar_data = np.transpose(lidar_data)
    ...
    def __getitem__(self, index):
        ...
        for index_t in self.indices[index]:
            ...
            imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose = self.get_single_item(
    ...
    #     # Initialize all lists as NumPy arrays
    #     all_imgs = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[0].shape), dtype=np.float32)
    #     all_rots = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[1].shape), dtype=np.float32)
    #     all_trans = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[2].shape), dtype=np.float32)
    #     all_intrins = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[3].shape), dtype=np.float32)
    #     all_lidar0_data = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[4].shape), dtype=np.float32)
    #     all_lidar0_extra = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[5].shape), dtype=np.float32)
    #     all_lidar_data = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[6].shape), dtype=np.float32)
    #     all_lidar_extra = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[7].shape), dtype=np.float32)
    #     all_lrtlist = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[8].shape), dtype=np.float32)
    #     all_vislist = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[9].shape), dtype=np.float32)
    #     all_tidlist = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[10].shape), dtype=np.int64)
    #     all_scorelist = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[11].shape), dtype=np.float32)
    #     all_seg_bev = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[12].shape), dtype=np.float32)
    #     all_valid_bev = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[13].shape), dtype=np.float32)
    #     all_center_bev = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[14].shape), dtype=np.float32)
    #     all_offset_bev = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[15].shape), dtype=np.float32)
    #     all_radar_data = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[16].shape), dtype=np.float32)
    #     all_egopose = np.empty((0, *self.get_single_item(self.indices[index][0], cams, refcam_id=refcam_id)[17].shape), dtype=np.float32)
    ...
    #         all_imgs = np.vstack((all_imgs, imgs[np.newaxis, ...]))
    #         all_rots = np.vstack((all_rots, rots[np.newaxis, ...]))
    #         all_trans = np.vstack((all_trans, trans[np.newaxis, ...]))
    #         all_intrins = np.vstack((all_intrins, intrins[np.newaxis, ...]))
    #         all_lidar0_data = np.vstack((all_lidar0_data, lidar0_data[np.newaxis, ...]))
    #         all_lidar0_extra = np.vstack((all_lidar0_extra, lidar0_extra[np.newaxis, ...]))
    #         all_lidar_data = np.vstack((all_lidar_data, lidar_data[np.newaxis, ...]))
    #         all_lidar_extra = np.vstack((all_lidar_extra, lidar_extra[np.newaxis, ...]))
    #         all_lrtlist = np.vstack((all_lrtlist, lrtlist[np.newaxis, ...]))
    #         all_vislist = np.vstack((all_vislist, vislist[np.newaxis, ...]))
    #         all_tidlist = np.vstack((all_tidlist, tidlist[np.newaxis, ...]))
    #         all_scorelist = np.vstack((all_scorelist, scorelist[np.newaxis, ...]))
    #         all_seg_bev = np.vstack((all_seg_bev, seg_bev[np.newaxis, ...]))
    #         all_valid_bev = np.vstack((all_valid_bev, valid_bev[np.newaxis, ...]))
    #         all_center_bev = np.vstack((all_center_bev, center_bev[np.newaxis, ...]))
    #         all_offset_bev = np.vstack((all_offset_bev, offset_bev[np.newaxis, ...]))
    #         all_radar_data = np.vstack((all_radar_data, radar_data[np.newaxis, ...]))
    #         all_egopose = np.vstack((all_egopose, egopose[np.newaxis, ...]))

    #     all_imgs = torch.from_numpy(all_imgs)
    #     all_rots = torch.from_numpy(all_rots)
    #     all_trans = torch.from_numpy(all_trans)
    #     all_intrins = torch.from_numpy(all_intrins)
    #     all_lidar0_data = torch.from_numpy(all_lidar0_data)
    #     all_lidar0_extra = torch.from_numpy(all_lidar0_extra)
    #     all_lidar_data = torch.from_numpy(all_lidar_data)
    #     all_lidar_extra = torch.from_numpy(all_lidar_extra)
    #     all_lrtlist = torch.from_numpy(all_lrtlist)
    #     all_vislist = torch.from_numpy(all_vislist)
    #     # all_tidlist = torch.from_numpy(all_tidlist)
    #     all_scorelist = torch.from_numpy(all_scorelist)
    #     all_seg_bev = torch.from_numpy(all_seg_bev)
    #     all_valid_bev = torch.from_numpy(all_valid_bev)
    #     all_center_bev = torch.from_numpy(all_center_bev)
    #     all_offset_bev = torch.from_numpy(all_offset_bev)
    #     all_radar_data = torch.from_numpy(all_radar_data)
    #     all_egopose = torch.from_numpy(all_egopose)
    ...
    #                 st = all_tidlist[t][i]
    #                 if st in all_tidlist[0]:
    #                     usable_tidlist[t, i] = np.where(all_tidlist[0] == st)[0][0]
    #                 else:
    #                     usable_tidlist[t, i] = counter
    #                     counter += 1
    #     all_tidlist = usable_tidlist
