...
class Bevformernet(nn.Module):
    ...
    def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, vox_util, rad_occ_mem0=None):
        ...
        if self.rand_flip:
            feat_camXs_8_ = torch.flip(feat_camXs_8_[self.rgb_flip_index], [-1])
            feat_camXs_16_ = torch.flip(feat_camXs_16_[self.rgb_flip_index], [-1])
            feat_camXs_32_ = torch.flip(feat_camXs_32_[self.rgb_flip_index], [-1])
        feat_camXs_8 = __u(feat_camXs_8_)
        feat_camXs_16 = __u(feat_camXs_16_)
        feat_camXs_32 = __u(feat_camXs_32_)
        ...
        for i in range(self.num_layers):
            ...
            bev_queries = self.self_attn_layers[i](bev_queries, bev_queries_pos.clone())
            ...
            # using only 1/8 feats, this works
            bev_queries = self.cross_attn_layers[i](bev_queries, 
                bev_keys[:, :5600], bev_keys[:, :5600], 
                query_pos=bev_queries_pos,
                reference_points_cam = reference_points_cam.detach(),
                spatial_shapes = spatial_shapes[:1].detach(), 
                bev_mask = bev_mask.detach(),
                level_start_index = level_start_index[:1].detach())

            # using only 1/16 feats, this works
            bev_queries = self.cross_attn_layers[i](bev_queries, 
                bev_keys[:, 5600:7000], bev_keys[:, 5600:7000], 
                query_pos=bev_queries_pos,
                reference_points_cam = reference_points_cam.detach(),
                spatial_shapes = spatial_shapes[1:2].detach(), 
                bev_mask = bev_mask.detach(),
                level_start_index = level_start_index[:1].detach())

            # using only 1/32 feats, this works
            bev_queries = self.cross_attn_layers[i](bev_queries, 
                bev_keys[:, 7000:], bev_keys[:, 7000:], 
                query_pos=bev_queries_pos,
                reference_points_cam = reference_points_cam.detach(),
