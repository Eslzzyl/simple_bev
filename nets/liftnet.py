...
class Liftnet(nn.Module):
    def __init__(self, Z, Y, X, ZMAX,
        ...
        if encoder_type == "res101":
            self.encoder = Encoder_res101(feat2d_dim + Z)
        elif encoder_type == "res50":
            self.encoder = Encoder_res50(feat2d_dim + Z)
        elif encoder_type == "effb0":
            self.encoder = Encoder_eff(feat2d_dim + Z, version='b0')
        else:
            # effb4
            self.encoder = Encoder_eff(feat2d_dim + Z, version='b4')
